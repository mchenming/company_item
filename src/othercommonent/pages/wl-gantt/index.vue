<template>
  <div class="wl-gantt" id="wl-gantt">
    <!-- 甘特图区 -->
    <div style="width:0;height:0;overflow:hidden">{{ganttTitleDate}}</div>
    <el-table
      ref="wl-gantt"
    ...
    >
      <template v-if="!ganttOnly">
        ...
        <template slot-scope="scope">
            <div>#{{scope.row.id}}_{{scope.row.name}}</div>
            <div style="position:absolute;left:200%;top:13px;width:150px;z-index:100;text-align:left">{{scope.row.status_name}}--{{scope.row.rate}}%</div>
        </template>
          <!-- <template slot-scope="scope">
            <el-input
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              :style="{width:dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-start' ?  setWidth(scope.row.startDate) : (dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-end'? setWidth((scope.row.endDate || scope.row.planDate || new Date().toLocaleDateString().replace(/\//g,'-')),'second') : (dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-later-end' ? setWidth(new Date().toLocaleDateString().replace(/\//g,'-'),'second') : '')), 
              left:dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-start'? setLeft(scope.row.startDate) : (dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-end'? '0': ''),
              background: dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-mid-item-end' ? setColor(scope.row.planDate) : ''}"></div>
              <div v-if="useRealTime" :class="realDayGanttType(scope.row, t.full_date, 'week')"></div>
            </template>
          </el-table-column>
          </el-table-column> -->
          <el-table-column :label="'abcd'" :render-header="setFuncLabel">
            <el-table-column
            class-name="wl-gantt-item"
            :resizable="false"
            :label="`${weekArr[0].week - 1}周`"
            :width="weekArr[0].date / 7 * 60"
          >
            <!-- :width="weekArr[0].date / 7 * 60" -->
            <template slot-scope="scope">
              <div class="wl-item-first" :style="{background: setfirstbgc(weekArr[0].full_date,scope.row)}">
              </div>
            </template>
          </el-table-column>
            <el-table-column
          ...
          >
            <template slot-scope="scope">
              <!-- wl-item-on wl-item-later-end -->
                <div :class="dayGanttType(scope.row, t.full_date, 'week')" v-if="dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-yjlater'" :style="{background:setBgc(t.full_date,scope.row)}"></div>
                <div :class="dayGanttType(scope.row, t.full_date, 'week')" v-else-if="dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-later-end'" :style="{background:setlaterBgc(t.full_date,scope.row)}"></div>
                <div :class="dayGanttType(scope.row, t.full_date, 'week')" v-else-if="dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-later'" :style="{background:setlaterBgcSecond(t.full_date,scope.row)}"></div>
                <div :class="dayGanttType(scope.row, t.full_date, 'week')" v-else-if="dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-full'" :style="{background:setlaterBgFull(t.full_date,scope.row)}"></div>
                <div :class="dayGanttType(scope.row, t.full_date, 'week')" v-else
                :style="{width:dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-start' ?  setWidth(scope.row.startDate, 'start') : (dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-end'? setWidth((scope.row.endDate || scope.row.planDate || new Date().toLocaleDateString().replace(/\//g,'-')),'second') : (dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-later-end' ? setWidth(new Date().toLocaleDateString().replace(/\//g,'-'),'second') : '')), 
                left:dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-start'? setLeft(scope.row.startDate) : (dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-item-end'? '0': ''),
                background: dayGanttType(scope.row, t.full_date, 'week') === 'wl-item-on wl-mid-item-end' ? setColor(scope.row) : ((!scope.row.planDate && !scope.row.endDate)? '#c9c9c9': ''),
                }"></div>
                <!-- background: (!scope.row.planDate && !scope.row.endDate)? '#c9c9c9': '', -->
            </template>
          </el-table-column>
          </el-table-column>
      </template>
...
import {
  ...
  flattenDeepParents,
  regDeepParents
} from '&p/util/array.js' // 导入数组操作函数
import ContextMenu from './components/wl-contextmenu'
import '&p/assets/css/clear.css'
...
export default {
  ...
  computed: {
    setfirstbgc () {
      return (fulltime, data) => {
        let lastTime = this.formatTime(fulltime)
        let startlen = new Date(data.startDate).getTime() // 项目开始时间
        let fulllen = new Date(lastTime).getTime() // 一个月开始第一个新一周的开始时间
        let sub = lastTime.substr(0, 8) + '01'
        let mouthlen = new Date(sub).getTime() // 一个月的第一天
        let days = this.getDaysBetween(sub, lastTime)
        let planlen = new Date(data.planDate).getTime()
        let endlen = new Date(data.endDate).getTime()
        // console.log(days)
        if (data.endDate) {
          if (startlen > mouthlen && startlen < fulllen) {
            if (endlen > fulllen) {
              let tian = this.getDaysBetween(sub, data.startDate)
              let width = parseInt((tian / days) * 100) + '%'
              return `linear-gradient(to right, #fff 0%, #fff ${width},#6d91d5 ${width},#6d91d5 100%)`
            }
          } else if (startlen < mouthlen && endlen < mouthlen) {
            return ''
          } else if (startlen < mouthlen && endlen >= mouthlen) {
            if (endlen <= fulllen) {
              let tian = this.getDaysBetween(sub, data.endDate)
              let width = parseInt((tian / days) * 100) + '%'
              return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${width},#fff ${width},#fff 100%)`
            } else if (endlen > fulllen) {
              // 开始小于月头 结尾大于第一周
              return '#6d91d5'
            }
          }
        ...
        if (startlen > mouthlen && startlen > fulllen) {
          return `linear-gradient(to right, #fff 0%, #fff 99%,#eee 99%,#eee 100%)`
        }
        if (startlen > planlen) {
          return '#f56c6c'
        }
        ...
        if (startlen >= mouthlen && startlen < fulllen) {
          ...
          if (planlen >= fulllen) {
            ...
            let tian = this.getDaysBetween(sub, data.startDate)
            // 蓝色
            let stian = this.getDaysBetween(sub, data.planDate)
            // let etian = this.getDaysBetween(data.planDate, lastTime)
            let width = parseInt((tian / days) * 100) + '%'
            let widthsecond = parseInt((stian / days) * 100) + '%'
            // 小于就一半蓝一半红
            return `linear-gradient(to right, #fff 0%, #fff ${width},#6d91d5 ${width},#6d91d5 ${widthsecond}, #f56c6c ${widthsecond},#f56c6c 100%)`
          }
        }

        if (startlen <= mouthlen) {
          if (!data.endDate && !data.planDate) {
            return '#c9c9c9'
          }
          if (planlen <= startlen) {
            // 全红
            return '#f5c6c6'
          } else if (planlen >= startlen && planlen <= fulllen) {
            let stian = this.getDaysBetween(sub, data.planDate)
            let widthsecond = parseInt((stian / days) * 100) + '%'
            // 一般蓝一半红
            return `linear-gradient(to right, #6d91d5 0%,#6d91d5 ${widthsecond}, #f56c6c ${widthsecond},#f56c6c 100%)`
          } else if (planlen >= fulllen) {
            // 全蓝
            return '#6d91d5'
          }
        }
        // 如果data.startDate> fulltime 白色
        // 如果小于fulltime且大于06-01 时 =》 当planDate<fulltime （一半蓝一半红） 当planDate大于fulltime时就是全蓝
        return ''
      }
    },
    setlaterBgFull () {
      return (tTime, row) => {
        tTime = this.formatTime(tTime)
        let startlen = new Date(row.startDate).getTime()
        let tlen = new Date(tTime).getTime()
        if (row.endDate) {
          let startday = this.oneDay(row.startDate)
          let endday = this.oneDay(row.endDate)
          let width = parseInt((startday / 7) * 100) + '%'
          let endwidth = parseInt((endday / 7) * 100) + '%'
          return `linear-gradient(to right, #fff 0%, #fff ${width},#6d91d5 ${width},#6d91d5 ${endwidth}, #fff ${endwidth}, #fff 100%)`
        } else if (!row.endDate && row.planDate) {
          let startday = this.oneDay(row.startDate)
          let planday = this.oneDay(row.planDate)
          let width = parseInt((startday / 7) * 100) + '%'
          let planwidth = parseInt((planday / 7) * 100) + '%'
          return `linear-gradient(to right, #fff 0%, #fff ${width},#6d91d5 ${width},#6d91d5 ${planwidth}, #f56c6c ${planwidth}, #f56c6c 100%)`
        } 
      }
    },
    setlaterBgcSecond () {
      return (tTime, row) => {
        ...
        let tlen = new Date(tTime).getTime()
        if (tlen < startlen) {
          let days = this.getDaysBetween(tTime, row.startDate)
          if (days <= 7) {
            console.log(days)
            let tian = this.oneDay(row.startDate)
            let width = parseInt((tian / 7) * 100) + '%'
            return `linear-gradient(to right, #fff 0%, #fff ${width},#f56c6c ${width},#f56c6c 100%)`
          } else {
            return `linear-gradient(to right, #fff 0%, #fff 99%,#eee 99%,#eee 100%)`
          }
        }
    ...
    setlaterBgc () {
      return (tTime, row) => {
        let startT = this.formatTime(tTime)
        let now = this.formatTime(new Date().toLocaleDateString().replace(/\//g, '-'))
        // console.log(this.getDaysBetween(startT, row.planDate))
        let days = this.getDaysBetween(startT, now)
        // 如果小于7则是末尾或者是当天在他们中间夹着
        if (days < 7) {
          if (new Date(startT).getTime() < new Date(row.planDate).getTime() && new Date(now).getTime() > new Date(row.planDate).getTime()) {
            let ti = this.oneDay(row.planDate)
            let endti = this.oneDay(now)
            let otherwidth = parseInt((ti / 7) * 100) + '%'
            let endwidth = parseInt((endti / 7) * 100) + '%'
            return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${otherwidth},#f56c6c ${otherwidth},#f56c6c ${endwidth}, #fff ${endwidth},#fff 100%)`
          } else {
            let ti = this.oneDay(now)
            let otherwidth = 7 - ti === 0 ? '0%' : parseInt((ti / 7) * 100) + '%'
            return `linear-gradient(to right, #f56c6c 0%, #f56c6c ${otherwidth},#fff ${otherwidth},#fff 100%)`
          }
        } else if (days > 7 && (new Date(row.planDate).getTime() <= new Date(now).getTime())) {
          // console.log('laterend')
          // let ti = this.oneDay(now)
          // let otherwidth = parseInt((ti / 7) * 100) + '%'
          let otherwidth = '100%'
          return `linear-gradient(to right, #f56c6c 0%, #f56c6c ${otherwidth},#fff ${otherwidth},#fff 100%)`
    ...
    setBgc () {
      return (tTime, row) => {
        ...
        let now = this.formatTime(new Date().toLocaleDateString().replace(/\//g, '-'))
        // console.log(this.getDaysBetween(startT, row.planDate))
        let days = this.getDaysBetween(startT, row.planDate)
        if (now === row.planDate) {
          let ti = this.oneDay(row.planDate)
          let otherwidth = 7 - ti === 0 ? '0%' : parseInt((ti / 7) * 100) + '%'
          // console.log('a')
          return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${otherwidth},#fff ${otherwidth},#fff 100%)`
        }
        // 如果小于7则是末尾或者是当天在他们中间夹着
        if (days < 7 && days !== 0) {
          if (new Date(startT).getTime() < new Date(now).getTime() && new Date(now).getTime() < new Date(row.planDate).getTime()) {
            let ti = this.oneDay(now)
            let endti = this.oneDay(row.planDate)
            let otherwidth = parseInt((ti / 7) * 100) + '%'
            let endwidth = parseInt((endti / 7) * 100) + '%'
            // console.log('zbc')
            return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${otherwidth},#c9c9c9 ${otherwidth},#c9c9c9 ${endwidth}, #fff ${endwidth},#fff 100%)`
          } else {
            let ti = this.oneDay(row.planDate)
            let otherwidth = 7 - ti === 0 ? '0%' : parseInt((ti / 7) * 100) + '%'
            return `linear-gradient(to right, #c9c9c9 0%, #c9c9c9 ${otherwidth},#fff ${otherwidth},#fff 100%)`
          }
        } else if (days > 7 && (new Date(startT).getTime() < new Date(now).getTime())) {
          // console.log('end')
          let ti = this.oneDay(now)
          let otherwidth = parseInt((ti / 7) * 100) + '%'
          return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${otherwidth},#c9c9c9 ${otherwidth},#c9c9c9 100%)`
        }
        return ''
      }
    },
    setColor () {
      return (data) => {
        let time = data.planDate
        let ti = this.oneDay(time)
        let width = 7 - ti === 0 ? '0%' : parseInt(((ti) / 7) * 100) + '%'
        if (data.endDate) {
          let tian = this.oneDay(data.endDate)
          let width = 7 - ti === 0 ? '100%' : parseInt(((tian) / 7) * 100) + '%'
          return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${width},#fff ${width},#fff 100%)`
        }
        return `linear-gradient(to right, #6d91d5 0%, #6d91d5 ${width},#f56c6c ${width},#f56c6c 100%)`
      }
    },
    setWidth () {
      return (time, type) => {
        let ti = this.oneDay(time)
        let width = 7 - ti === 0 ? '100%' : parseInt(((7 - ti) / 7) * 100) + '%'
        if (type === 'start') {
          width = 7 - ti === 0 ? '101%' : parseInt(((7 - ti) / 7) * 100) + 1 + '%'
        }
        let otherwidth = 7 - ti === 0 ? '100%' : parseInt((ti / 7) * 100) + '%'

        // console.log(width)
        if (type === 'second') return otherwidth
        return width
      }
    },
    setLeft () {
      return (time) => {
        let ti = this.oneDay(time)
        let width = 7 - ti === 0 ? '100%' : ((7 - ti) / 7) * 100 + '%'
        let left = 100 - (7 - ti === 0 ? '100' : parseInt(((7 - ti) / 7) * 100)) + '%'
        // console.log(width)
        return left
      }
    },
    ...
    ganttTitleDate () {
      ...
      let end_date_spilt = dayjs(this.self_end_date)
        .format('YYYY-M-D')
        .split('-')
        // console.log(start_date_spilt,end_date_spilt)
      ...
      if (this.self_date_type === "yearAndWeek") {
        // console.log(this.yearAndWeekTitleDate(
        //   start_year,
        //   start_mouth,
        //   end_year,
        //   end_mouth
        ...
        console.log(this.mouthAndDayTitleDate(
          start_year,
          start_mouth,
          end_year,
          end_mouth
        ...
        console.log(this.yearAndMouthTitleDate(
          start_year,
          start_mouth,
          end_year,
          end_mouth
  ...
  methods: {
    ...
    setFuncLabel (h, {column}) {
      // console.log(this.weekArr)
      let day = this.weekArr[0].full_date.split('-')[2]
      // console.log(day)
      let num = 30
      if (day - 0 === 3) {
        num = 0
      ...
      for (let i = 0; i < this.labelArr.length; i++) {
        arr.push(h('div', {style: 'flex:1;border-right:1px solid #fff;padding:7px 0;'}, this.labelArr[i]))
      }
      return h('div', {
        style: `display:flex;padding-right:${num}px;`
      }, arr)
    },
    /**
     * 开始时间改变
    ...
    yearAndWeekTitleDate(start_year, start_mouth, end_year, end_mouth) {
      ...
      console.log(start_mouths.concat(end_mouths))
      ...
      if (week) {
        ...
        for (let i = _day; i < dates_num; i += 7) {
          days.push({
            ...
            week: this.getYearWeek(`${year}-${month}-${i}`)
          });
        }
      } else {
    ...
    dayGanttType (row, date, unit = 'days') {
      ...
      // let nowDate = '2020-07-04'
      if (!end_date) {
        // console.log(22)
        end_date = nowDate
      // 当前时间大于验收计划完成时间 且需求不是已关闭状态
        let laterbetween
      if (row.planDate && new Date(nowDate).getTime() > new Date(row.planDate).getTime()) {
        end_date = row.planDate
        laterbetween = dayjs(date).isBetween(row.planDate, nowDate, unit)
        let end = dayjs(nowDate).isSame(date, unit)
        if (laterbetween) {
          return 'wl-item-on wl-item-later'
        }
        if (end) {
          return 'wl-item-on wl-item-later-end'
        }
      } else if (new Date(nowDate).getTime() < new Date(row.planDate).getTime()) {
        end_date = nowDate
        let yujibetween = dayjs(date).isBetween(row.planDate, nowDate, unit)
        let end = dayjs(row.planDate).isSame(date, unit)
        if (yujibetween) {
          return 'wl-item-on wl-item-yjlater'
        }
        if (end) {
          return 'wl-item-on wl-item-yjlater'
        }
      ...
      // if (row.planDate) {
      //   end = dayjs(row.planDate).isSame(date, unit);
      // }
      if (start && end) {
        // console.log('full')
      ...
      if (end) {
        // console.log('end')
        if (row.planDate && new Date(nowDate).getTime() > new Date(row.planDate).getTime()) {
          return 'wl-item-on wl-mid-item-end'
        } else if (new Date(nowDate).getTime() < new Date(row.planDate).getTime()) {
          return 'wl-item-on wl-item-yjlater'
        }
        // console.log(end_date)
    ...
    handleMouseEnter(row, column, cell, event) {
      ...
      if (column.label !== '需求') {
        this.timer = setTimeout(() => {
          this.handleRowContextMenu(row,column,event)
        }, 500)
      }
    ...
    handleMouseLeave(row, column, cell, event) {
      ...
      // document.querySelector('.ft-context-menu').style.display = 'none'
    ...
    handleRowContextMenu(row, column, event) {
      ...
      this.contextMenu.y = event.pageY + 30;
      this.contextMenu.show = true;
      if (this.contextMenu.x > (document.body.clientWidth-500)) {
        this.contextMenu.x = event.x - 200
      }
    }, // 当某一行被鼠标右键点击时会触发该事件
    handleContextmenu() {
      console.log('右击')
...
<style scoped>
  .wl-gantt >>> .el-table tbody tr:hover>td { 
    background-color:transparent !important
  }
.wl-gantt >>> .el-table th{
  padding: 0;
}
.wl-gantt >>> .el-table .cell, .el-table--border td:first-child .cell, .el-table--border th:first-child .cell{
  padding-left: 0;
}
</style>
