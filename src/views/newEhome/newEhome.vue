        ...
        <el-row>
          <el-col :span="6" id="thchange">
              <el-table class="el-table_header"  ref="multipleTable" :data="listData" v-loading="listLoading" element-loading-text="加载中。。。"
                tooltip-effect="dark" style="width: 100%;margin-left:0px;height:240px;font-size: 11px;text-align:center; overflow-y:auto;"
                ...
                <el-table-column align="center" label="状态" min-width="25%" >
                  <template slot-scope="scope" >
                    <div v-if="scope.row.status === '稳定'">
                      <div style="width: 50px;height: 25px;background-color: #42b165;text-aline:center;line-height:25px;border-radius:5px;margin:auto">
                        <span style="color: #ffffff">{{scope.row.status}}</span>
                      </div>
                    </div>
                    <div v-if="scope.row.status === '异常'">
                        <el-tooltip placement="right-start">
                          <div slot="content"><span v-html="scope.row.erroMess"></span></div>
                          <div style="width: 50px;height: 25px;background-color: #f56c6c;text-aline:center;line-height:25px;border-radius:5px;margin:auto">
                              <span style="color: #ffffff">{{scope.row.status}}</span>
                          </div>
                        </el-tooltip>
                    </div>
          ...
          <el-col :span="3"  class="progresscss" style="position:relative;">
            <h2>服务SLA</h2>
              <div class="progresspart" tag="div" @click="savetime">
                ...
                <el-progress type="circle" class="slacss" :stroke-width=strokeWidth :width=progressWidth :percentage='Number(slaPassrate)' :show-text='false'></el-progress>
                <div class="xd">
                  <div class="boxshow">
          ...
          <el-col :span="3"  class="progresscss">
            <h2>业务场景</h2>
              <div class="progresspart" @click="business">
                ...
                <el-progress type="circle" class="busscss" :stroke-width=strokeWidth :width=progressWidth :percentage='Number(bussPassrate)' :show-text='false'>成功率</el-progress>
                <div class="xd">
                  <div class="boxshow">
          ...
          <el-col :span="3"  class="progresscss">
            <h2>节点异常</h2>
              <div class="progresspart" @click="errnode">
                <div class="passTextCss">
                  <span>超过1s比例</span>
                  <span v-if="machinePassrate>0.6" style="color:red">{{machinePassrate}}%</span>
                  <span v-if="machinePassrate<0.6" style="color:#000">{{machinePassrate}}%</span><br/>
                  <span>错误率</span><br/>
          ...
          <el-col :span="3"  class="progresscss">
            <h2>Crash数据</h2>
              <!-- <div class="progresspart" @click="crashdata"  ref="crashData" id="crashData" :class="{cushow : crashAuthor===0}"> -->
              <div class="progresspart" @click="crashdata"  ref="crashData" id="crashData">
                <div class="passTextCss">
                  <span>Crash率</span>
                  <span v-if='crashPassrate>=0.05' style="color:red;display:block;">{{crashPassrate}}%</span>
                  <span v-if="crashPassrate<0.05" style="color:#000;display:block;">{{crashPassrate}}%</span>
                  <span>影响人数</span>
                  <p style="margin:0;" v-if="crashAuthor<20">{{crashAuthor}}</p>
                  <p style="margin:0;color:red" v-if="crashAuthor>20">{{crashAuthor}}</p>
                </div>
                <el-progress type="circle" class="crashcss" :stroke-width=strokeWidth :width=progressWidth :percentage='Number(crashPassrateP)' :show-text='false'></el-progress>
                <div class="xd">
                  <div class="boxshow">
        ...
        <el-row style="margin-top:10px;">
          <el-col :span="6" style="margin-right:12px;">
            <div style="height: 30px;background-color: white;width: 100%;">
              ...
              <b style="margin-left: 15px;font-size: 16px;">业务场景(成功率)</b>
            </div>
          ...
          <el-col :span="9">
            <div style="height: 30px;background-color: white;width: 98%;cursor:pointer;" @click="jumptopage">
              <div style="height: 13px;background-color: white;width: 100%;"></div>
              <b style="margin-left: 15px;font-size: 16px;">模块访问量Top5</b>
            </div>
          ...
          <el-col :span="9" style="cursor:pointer">
            <div style="height: 30px;background-color: white;width: 100%;"  @click="errnode">
              <div style="height: 13px;background-color: white;width: 100%;"></div>
            ...
            <el-table ref="multipleTable" :data="nodeDataList" v-loading="listLoading" element-loading-text="加载中。。。"
              ...
              <el-table-column align="center" min-width="62" label="错误率">
                <template slot-scope='scope'>
                  <!-- {{erroratet}} -->
                  <div v-if="Number(scope.row.request_error_rate.replace('%',''))>6" style="width: 60px;height: 28px;line-height:28px;background-color: #fef1f1;margin:auto;border-radius:5px;">
              ...
              <el-table-column align="center" min-width=76  label="平均时间">
                <template slot-scope='scope'>
                  <span v-if="scope.row.request_time">{{(Number(scope.row.request_time.split('.')[0])/(scope.row.request_num-scope.row.request_error_num)).toFixed(2)}}ms</span>
                </template>
              </el-table-column>
              ...
              <el-table-column align="center" min-width=68  label="占比">
                <template slot-scope='scope'>
                  <div>
                    <span v-if="(parseFloat(scope.row.low_request_num/scope.row.request_num)*100)>0.6" style="color: red">{{(parseFloat(scope.row.low_request_num/scope.row.request_num)*100).toFixed(3)}}%</span>
        ...
        <el-row style="margin-top:10px;">
          ...
          <el-col :span="9">
            <div style="height: 30px;background-color: white;width: 98%;">
              <div style="height: 13px;background-color: white;width: 100%;"></div>
              <b style="margin-left: 15px;font-size: 16px;">启动数&设备数</b>
            </div>
            <div style="background-color: white;width:98%;height:240px;">
          ...
          <el-col :span="9" id="changeColor">
            <div class="waitres" style="cursor:pointer" @click="eproblem">待解决&nbsp;&nbsp;&nbsp;{{productionPassrate}}</div>
            ...
            <el-table ref="multipleTable" :data="productionFeedbackList" height="270" v-loading="listLoading" element-loading-text="加载中。。。"
              ...
              <el-table-column align="left" width=80 label="生产问题" type="index">
                <template slot-scope='scope'>
                  <div style="text-align:left;cursor:pointer;color:#4b78cd" @click="goRedmine(scope.row[1])">#{{scope.row[1]}}</div>
                </template>
              </el-table-column>
...
getnewcrashnumsummary, getfeedbackdatalistv2, getfeedbackstaticlistv2, getdauitemdata, getdomainallsummarydata} from '@/api/newEhomeJS/eHomePort'
...
export default {
  ...
  data () {
    return {
      ...
      listData: [
        ...
        {indtype: '模块访问量', erroNum: '0', successNum: '0', status: '稳定'},
  ...
  created () {
    ...
    if (this.$route.query.stime || this.$route.query.etime) {
      ...
      // this.queryTime = [s, e]
      this.$set(this.queryTime, 0, s)
      this.$set(this.queryTime, 1, e)
    ...
    // console.log(this.$route.query.isKey)
    if (typeof (this.$route.query.isKey) !== 'undefined' && this.$route.query.isKey !== '') {
      if (this.$route.query.isKey === 'true' || this.$route.query.isKey === true) {
        this.isKey = true
      } else if (this.$route.query.isKey === 'false' || this.$route.query.isKey === false) {
        this.isKey = false
      }
    }
    // this.searchData()
    this.firstGetData(str)
  },
  mounted () {
  ...
  methods: {
    ...
    crashdata () {
      ...
      // if (this.isKey !== true) {
      //   this.$router.push({path: '/newcrashdata', query: row})
      // } else if (this.isKey === true && Number(this.crashAuthor) !== 0) {
      //   this.$router.push({path: '/newcrashdata', query: row})
      // }
      this.$router.push({path: '/newcrashdata', query: row})
    },
    errnode () {
      ...
      this.$router.push({path: '/newerrnode', query: row})
    },
    eproblem () {
      localStorage.setItem('business', this.queryTime)
      localStorage.setItem('biskey', this.isKey)
      ...
      this.$router.push({path: '/neweproblem', query: row})
    },
    business () {
      localStorage.setItem('business', this.queryTime)
      localStorage.setItem('biskey', this.isKey)
      ...
      this.$router.push({path: '/newebusiness', query: row})
    },
    savetime () {
      ...
      this.$router.push({path: '/newesal', query: row})
    },
    errletsh (row) {
      if (row.erroNum !== 0) {
        this.tips = true
    ...
    getPvuvData (type) {
      ...
      getdauitemdata(row).then(response => {
        ...
        if (response.data.data !== null) {
          purow.xAxisData = response.data.data.timeList
    ...
    getfeedbackdatalist (projectId, month, ftype) {
      ...
      // productionRow.static_month = '2019-10-01'
      ...
      getfeedbackdatalistv2(productionRow).then(response => {
        if (response.data.data && response.data.data.length > 0) {
          ...
          response.data.data.forEach((item, index) => {
            ...
            this.productionFeedbackList.push(arr)
          })
        }
      })
    },
    getPassrateData () {
      ...
      getallsummarydata(this.listQuery, this.isKey).then(response => {
        ...
        for (var j in dataobj) {
          if (Number(dataobj[j].summary_sla.value) == 0) {
            dataobj[j].summary_sla.value = 1
          }
          dataobj[j].avg_time_cost.value = Number(dataobj[j].avg_time_cost.value).toFixed(2) + 'ms'
          dataobj[j].summary_sla.value = Number(dataobj[j].summary_sla.value * 100).toFixed(2) + '%'
        }
        // console.log(dataobj)
        this.sladata = dataobj
        getallsummaryscenedata(this.listQuery, this.isKey).then(response => {
          ...
          if (bussData !== null) {
            ...
            if (!this.isKey) {
              if (this.sladata.QueryTokenInfoAction) {
        ...
        var slaPassrate
        if (this.sladata.all) {
          slaPassrate = this.sladata.all.summary_sla.value
          this.slatime = this.sladata.all.avg_time_cost.value
        } else {
          slaPassrate = 0
          this.slatime = 100
        }
        // console.log(slaPassrate)
        this.slaPassrate = slaPassrate.substring(0, slaPassrate.lastIndexOf('%')) - 0
        if (Number(this.slaPassrate) === 0) {
          this.slaPassrate = 100
        }
        let erroNum = 0
        var erroMess = ''
        var boo = false
        ...
        if (this.sladata.auth.avg_time_cost.value.replace('ms', '') > 200 || this.sladata.auth.summary_sla.value.replace('%', '') < 99) {
          ...
          if (this.sladata.auth.avg_time_cost.value.replace('ms', '') > 200) {
            ...
            erroMess += '登录接口成功率低于99%，\n'
          }
        }
        if (this.sladata.checkrange.avg_time_cost.value.replace('ms', '') > 70 || this.sladata.checkrange.summary_sla.value.replace('%', '') < 99) {
          erroNum++
          boo = true
          if (this.sladata.checkrange.avg_time_cost.value.replace('ms', '') > 70) {
            erroMess += '打卡定位接口平均响应时间高于70ms，\n'
          } else if (this.sladata.checkrange.avg_time_cost.value.replace('ms', '') > 70 && this.sladata.checkrange.summary_sla.value.replace('%', '') < 99) {
            erroMess += '打卡定位接口平均响应时间高于70ms且接口成功率低于99%，\n'
          } else {
            erroMess += '打卡定位接口成功率低于99%，\n'
          }
        }
        if (this.sladata.ischeck.avg_time_cost.value.replace('ms', '') > 70 || this.sladata.ischeck.summary_sla.value.replace('%', '') < 99) {
          erroNum++
          boo = true
          if (this.sladata.ischeck.avg_time_cost.value.replace('ms', '') > 70) {
            erroMess += '打卡状态接口平均响应时间高于70ms，\n'
          } else if (this.sladata.ischeck.avg_time_cost.value.replace('ms', '') > 70 && this.sladata.ischeck.summary_sla.value.replace('%', '') < 99) {
            erroMess += '打卡状态接口平均响应时间高于70ms且接口成功率低于99%，\n'
          } else {
            erroMess += '打卡状态接口成功率低于99%，\n'
          }
        }
        if (this.sladata.workcheck.avg_time_cost.value.replace('ms', '') > 100 || this.sladata.workcheck.summary_sla.value.replace('%', '') < 99) {
          erroNum++
          boo = true
          if (this.sladata.workcheck.avg_time_cost.value.replace('ms', '') > 100) {
            erroMess += '打卡接口平均响应时间高于100ms，\n'
          } else if (this.sladata.workcheck.avg_time_cost.value.replace('ms', '') > 100 && this.sladata.workcheck.summary_sla.value.replace('%', '') < 99) {
            erroMess += '打卡接口平均响应时间高于100ms且接口成功率低于99%，\n'
          } else {
            erroMess += '打卡接口成功率低于99%，\n'
          }
        }
        if (this.sladata.all.avg_time_cost.value.replace('ms', '') > 200 || this.sladata.all.summary_sla.value.replace('%', '') < 99) {
          erroNum++
          boo = true
          if (this.sladata.all.avg_time_cost.value.replace('ms', '') > 200) {
            erroMess += '所有接口（统计）平均响应时间超过200ms，\n'
          } else {
        ...
        // console.log(erroNum)
        // if (this.isKey === false) {
        //   this.listData[0].successNum = 8 - this.listData[0].erroNum
        // } else {
        //   this.listData[0].successNum = 7 - this.listData[0].erroNum
        // }
        this.listData[0].successNum = 7 - this.listData[0].erroNum
      })
      const productionRow = {}
      ...
      getfeedbackstaticlistv2(productionRow).then(response2 => {
        ...
        if (response2.data.data.length > 0) {
          ...
          d[0] = datemouth
          d[1] = '6'
          if (response2.data.data[0].static_month.replace(/-/g, '') === datemouth) {
            djj1 = response2.data.data[0].total_unclose_feedback_num
            byxz1 = response2.data.data[0].new_feedback_num
          }
          this.getfeedbackdatalist(productionRow.project_id, d[0], d[1])
        }
        productionRow.project_id = '27'
        getfeedbackstaticlistv2(productionRow).then(response1 => {
          ...
          if (response1.data.data.length > 0) {
            ...
            d1[0] = datemouth
            d1[1] = '6'
            //  d[0] = response.data.data[0].static_month
            //  d[1] = ''
            this.getfeedbackdatalist(productionRow.project_id, d1[0], d1[1])
            if (response1.data.data[0].static_month.replace(/-/g, '') === datemouth) {
              djj2 = response1.data.data[0].total_unclose_feedback_num
              byxz2 = response1.data.data[0].new_feedback_num
            }
    ...
    getCrashData () {
      ...
      getcrashnumsummary(obj).then(response => { // 获取crash数
        // console.log(response)
        ...
        if (data !== null) {
          ...
          getdausummarydata(obj).then(res => {
            // console.log(res.data.data)
            if (res.data.data !== null) {
              if (res.data.data.android_pv === null) {
                res.data.data.android_pv = 0
              }
              if (res.data.data.iphone_pv === null) {
                res.data.data.iphone_pv = 0
              }
              this.listData[4].successNum = (res.data.data.android_pv - 0) + (res.data.data.iphone_pv - 0)
              if (Number(this.listData[4].successNum) !== 0) {
                this.crashPassrate = (erroNum * 100 / this.listData[4].successNum).toFixed(3)
                // console.log(erroNum)
                // console.log(1)
              } else {
                this.crashPassrate = 0
              }
            }
          })
          getdomainallsummarydata(obj).then(res => {
            if (res.data.data.error_rate !== null) {
              // console.log(res.data.data)
        ...
        getcrashuvsummary(obj).then(response => { // 获取CRASH影响用户数
          const data = response.data.data
          // console.log(data)
          var erroMess1 = ''
          if (data !== null) {
          ...
          getnewcrashnumsummary(obj).then(response => { // 获取新增崩溃类型数
            // console.log(response.data.data)
            const data = response.data.data
            androidData[2] = 0
            iosData[2] = 0
      ...
      getnodeaccesssummary(this.listQuery).then(response => {
        ...
        if (response.data.data !== null) {
          ...
          for (let i = 0; i < response.data.data.length; i++) {
            ...
            erroRateNum += element.request_error_num - 0
            let rate = (element.low_request_num - 0) * 100 / (element.request_num - 0)
            this.overrate = false
            this.erroratet = false
            if ((element.request_error_rate.replace('%', '') - 0) > 6 || rate > 0.6) {
              erroNum++
              boo = true
              // console.log(element.request_error_rate)
              if ((element.request_error_rate.replace('%', '') - 0) > 6) {
                erroMess += element.node_id + '错误清理率高于6%，'
                this.erroratet = true
                // console.log(this.erroratet)
              } else if ((element.request_error_rate.replace('%', '') - 0) > 6 && rate > 0.6) {
                erroMess += element.node_id + '错误清理率高于6%且超时1s请求占比高于0.6%，'
                this.overrate = true
                this.erroratet = true
              } else {
                erroMess += element.node_id + '超时1s请求占比高于0.6%，'
                this.overrate = true
              }
            }
