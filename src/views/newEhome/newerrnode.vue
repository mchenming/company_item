...
import { getonlinedata, getnodeaccessrequestnumdetail, getnodeaccesserrorratedetail, getnodeaccessrequesttimedetail, getnodeaccesslowrequestnumdetail } from '@/api/newEhomeJS/errnode'
export default {
  data () {
    return {
      changeTime: [],
    ...
    getechartdata () {
      ...
      getonlinedata(obj).then(res => {
        ...
        getnodeaccessrequesttimedetail(obj).then(res => {
          ...
          resdata.titledata = (Number(Arrdata[0].request_time.split('.')[0]) / (Arrdata[0].request_num - Arrdata[0].request_error_num)).toFixed(2) + 'ms'
          if (Number(resdata.titledata.split('ms')[0]) > 70) {
        ...
        getnodeaccessrequesttimedetail(obj1).then(res => {
          ...
          // console.log(resdata)
          resdata.title = '平均响应时间:'
          resdata.titledata = (Number(Arrdata[1].request_time.split('.')[0]) / (Arrdata[1].request_num - Arrdata[1].request_error_num)).toFixed(2) + 'ms'
          if (Number(resdata.titledata.split('ms')[0]) > 70) {
            resdata.titlecolor = 'red'
          } else {
            resdata.titlecolor = 'black'
        ...
        getnodeaccessrequesttimedetail(obj2).then(res => {
          ...
          // console.log(resdata)
          resdata.title = '平均响应时间:'
          resdata.titledata = (Number(Arrdata[2].request_time.split('.')[0]) / (Arrdata[2].request_num - Arrdata[2].request_error_num)).toFixed(2) + 'ms'
          if (Number(resdata.titledata.split('ms')[0]) > 70) {
            resdata.titlecolor = 'red'
          } else {
            resdata.titlecolor = 'black'
