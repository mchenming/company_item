                ...
                <div style="background-color:#fff;width:70%;margin:auto;min-height:100vh">
                    ...
                    <div class="m_content">
                        <div style="display:flex;align-items:center;font-size:14px;margin:10px 0;">
                          ...
                          <div style="border:1px solid #ccc;padding:7px 5px;flex:1;font-size:14px;min-height:32px;color:#00f;background-color:#f5f7fa">{{formLabelAlign.subject}}-{{formLabelAlign.handleBranch}}-{{formLabelAlign.handleAuthor}}-<span v-if="formLabelAlign.createdOn">{{formLabelAlign.createdOn.substr(0,10)}}</span>
                          </div>
                        </div>
                        ...
                        <div style="display:flex;align-items:center;font-size:14px;margin:10px 0;">
                          ...
                          <div style="flex:1;display:flex;align-items:center;margin-left:10px;" class="needlevel">
                            <el-form-item label="需求实现满意度" prop="reviewLevel" style="width:100%;display:flex;margin-bottom:0;">
                              <el-select v-model="sendInfo.reviewLevel" style="flex:1;width:100%;" :disabled="isShow" placeholder="" popper-class="notop">
                                <el-option
                          ...
                          <el-form-item label="评价总结" prop="messageDetail" >
                              <el-input type="textarea" autosize :maxlength=500 placeholder="【请结合“需求应用成效评价标准”总结需求上线后的应用效果，例如需求相关系统功能的使用情况等】"  v-model="sendInfo.messageDetail" style="width:100%;" :disabled="isShow"></el-input>
                          </el-form-item>
            ...
            <el-col :span="5" style="height:100vh;">
                <div style="height:100vh;display:flex;flex-flow:column;">
                    <div style="background-color:#fff;padding:0px 0px 0 10px;height:48%;">
                        ...
                        <div style="border:1px solid #ccc;padding:50px 30px;height:100%;position:relative;border-bottom:none;">
                          ...
                          <div style="text-align:right;position:absolute;bottom:90px;right:30px;">
                            <el-button style="border:1px solid #aaa" @click="submit" v-if="Number(this.routedata.openType) === 0" :disabled="isShow" :class="{hasColor :!isShow}">提交</el-button>
                            <el-button class="fankui" style="border:1px solid #aaa" @click="confirmSure" v-if="Number(this.routedata.openType) === 1" :disabled="subFlag" :class="{hasColor :!subFlag}">确认</el-button>
                          </div>
                        </div>
                    ...
                    <div style="height:52%;padding:0 0 0 10px;background-color:#fff;overflow-y:auto;">
                      <div style="padding:15px 10px;background-color:#fff;border:1px solid #ccc;min-height:100%;border-bottom:none;">
                          <p>变动历史</p>
                          <div>
                          ...
                          <div v-if="sendInfo.history">
                            <div v-for="(item,index) in sendInfo.history" :key="index">
                              ...
                              <div style="margin:8px 0;font-size:14px;">
                                <div v-if="item.add_type==='知会'">{{item.create_author}}确认了需求成效评价的知会</div>
                                <div v-else>{{item.create_author}}{{item.add_type}}了需求成效评价</div>
                              </div>
                            </div>
                          </div>
                      </div>
...
import {addHistorySecond} from '@/api/EmailPost/previewSecond.js'
...
export default {
  ...
  created () {
    ...
    //   this.$route.query.openType = this.$route.query.openType
    // }
    var url = window.location.href
    if (url.indexOf('&token=') !== -1) {
      this.loginEiac()
    }
    let obj = JSON.parse(localStorage.getItem('routedata'))
    this.routedata = obj
  ...
  methods: {
    loginEiac () {
      var url = window.location.href
      if (url.indexOf('&token=') !== -1) {
        localStorage.setItem('routedata', JSON.stringify(this.$route.query))
        this.$cookie.set('token', this.$route.query.token)
        this.$router.push('/previewSecond')
      }
    },
    ...
    confirmSure: _.throttle(function () {
      this.$refs.formName.validate((vaild) => {
        if (vaild) {
          this.sendInfo.needId = this.routedata.need_id
          this.sendInfo.needPurpose = this.need_purpose.join(',')
          if (!this.sendInfo.createTime) {
            this.getOneHoursTime()
          }
          var sendobj = this.sendInfo
          sendobj.openType = this.routedata.openType
          sendobj.token = this.$cookie.get('token')
          if (this.routedata.token) {
            sendobj.createAuthorId = this.routedata.token
          } else {
            sendobj.createAuthorId = this.$cookie.get('token')
          }
          if (this.routedata.toduType) {
            sendobj.toduType = this.routedata.toduType
          }
          addHistorySecond(sendobj).then(res => {
            if (res.data.result === 'success') {
              if (this.routedata.toduType) {
                if (Number(this.routedata.toduType) === 1) {
              ...
              this.$message({type: 'error', message: res.data.result, duration: 30000, showClose: true})
              // setTimeout(() => {
              //   window.opener = null
              //   window.open('', '_self')
              //   window.close()
    ...
    getdata () {
      ...
      yuluan(row).then(response => {
        ...
        getQxNeedmeritsReview(row).then(res => {
          ...
          if (Number(this.routedata.openType) === 0) {
            if (res.data.hasOwnProperty('data') && res.data.data === null) {
              // 可以填一次底下不置灰
              this.isShow = false
              this.huiguKind = '待回顾'
          ...
          if (Number(this.routedata.openType) === 1) {
            this.isShow = false
            this.huiguKind = '回顾待确认'
            if (this.routedata.toduType) {
              if (Number(this.routedata.toduType) === 1) {
                // 不能修改
                // console.log(123)
                // this.$refs.fankui.innerText = '反馈'
                document.querySelector('.fankui').innerText = '反馈知会'
                this.isShow = true
              }
            }
          ...
          // res.data.data.type = 'n'
          if (res.data.hasOwnProperty('data') && res.data.data !== null) {
            if (res.data.data.type) {
              if (res.data.data.type === 'y') {
                // this.isShow = false
                this.subFlag = false
              } else if (res.data.data.type === 'n') {
                this.isShow = true
                this.subFlag = true
              }
            }
...
<style lang="scss">
    ...
    .notop .el-select-dropdown__wrap,.el-scrollbar__wrap{
